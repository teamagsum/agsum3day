...
/*20140606*/
	if ( (is_home() or is_category()) && is_user_logged_in() ) {
...
function prologue_get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	...
	//escape by PM HS ("likeが最下のコメントに付かない対策")
	/*
	...
	*/
	...
	if ( get_option( 'comment_registration' ) && !$user_ID )
		...
		$link = "<!-- comment-reply-link --><a rel='nofollow' class='comment-reply-link' href='". get_permalink($post). "#" . urlencode( $respond_id ) . "' title='". __( 'Reply', 'p2' )."' onclick='return addComment.moveForm(\"" . esc_js( "$add_below-$comment->comment_ID" ) . "\", \"$comment->comment_ID\", \"" . esc_js( $respond_id ) . "\", \"$post->ID\")'>$reply_text</a><!-- /comment-reply-link -->";
...
/*--------------------------
 * JS読み込み（iphoneのはいつでも読み込むように）
 */
add_action( 'wp_enqueue_scripts', 'pm_enqueue_scripts', 1000 );
function pm_enqueue_scripts() {
	...
	if(is_home() or is_front_page() or is_category()){
		...
		//jquery.jTaggingの読み込み/*20140606*/
		wp_enqueue_script(
			'jquery.jTagging',
			get_stylesheet_directory_uri() . '/js/jquery.jTagging.js',
			array('jquery'),
			'20121124',
			true
		);
		...
		//分割タグ関連jsの読み込み
		wp_enqueue_script(
			'pm_more-link-ajax-load',
			get_stylesheet_directory_uri() . '/js/pm_more-link-ajax-load.js',
			array('jquery'),
			'20140709',
			true
		);

		//TOPのソーティング選択表示jsの読み込み
		wp_enqueue_script(
			'pm_sort_and_tagfiters',
			get_stylesheet_directory_uri() . '/js/pm_sort_and_tagfiters.js',
			array('jquery'),
			'20140709',
			true
		);
...
<div id="tagfilter">
<span>除外対象：</span><a href="#" id="tag_all">なし</a>｜<a href="#" id="tag_notin--ホワイトボード-勤怠-電話">"ホワボ、勤怠,電話"以外</a>｜<a href="#" id="tag_notin--勤怠">"勤怠"以外</a>
</div>
</div>
EOF;
}
...
function pm_sort_and_tagfiters( $query ) {
	...
	if ( $query->is_home() || $query->is_category() ) {
  		 ...
  		 //コメント日時もソート時に考慮するか？
		if ( isset( $_COOKIE["wppm_search_order"] ) && $_COOKIE["wppm_search_order"] == "last_activity"){
			$query->set( 'orderby_last_activity', 1 );
		}

		//タグによるfilter：クッキーの値は『tag_notin--勤怠-電話』（filter--tag-tag-...）
		if ( isset( $_COOKIE["wppm_tagfilter"] ) &&  $_COOKIE["wppm_tagfilter"] != "tag_all"){
...
/*--------------------------
 * Admin Barの無効化（管理人 or ネットワーク管理者のみ表示）
 */
...
function pm_TagInputHelper() {
	...
	if($bufSP != ""){
		$html_bufSP = "		<div class='TagSPgroup'><span>特殊タグ</span>$bufSP</div>";
	}
	if($bufML != ""){
...
function email_to_ML($post_ID)  {
	...
	if($post->post_status == 'publish') {
		...
		if ($to != "" ) {
			...
			$headers[] = 'From: yaritori <'.get_option('admin_email').'>';
			//TEST PM HS 20140214
...
/*
 * 未試用widgetリストがあれば消す（常にリセットされてしまうのでNG）----> 一回、widgetを操作（追加編集削除）するとなくなる、、、
 */
//if( !isset($widgets['wp_inactive_widgets']) || $widgets['wp_inactive_widgets']==NULL ){
//if( count($widgets['wp_inactive_widgets']) > 0 ){
/*if(is_admin()){
...
/*
 * 変換されたembedURLを強制的にhttpsに
 * ※最終的に作られたhtmlに対してfilter
 */
function pm_filter_oembed_dataparse($html){
	$html = str_replace('http:', 'https:',$html);
	return $html;
}
add_filter( 'oembed_dataparse', 'pm_filter_oembed_dataparse',10,1 );


/*
 * コメントの表示順番（上から古い順）20140627
 */
update_option( 'comment_order', 'asc');


/*
 * logoutリンクに_wpnonceを追加 140713　（確認画面を出さないように）
 */
add_action( 'wp_footer', 'pm_correct_logoutLink');
function pm_correct_logoutLink(){
?>
<script type="text/javascript">
(function($) {
	var logoutURLOrg = "<?php echo wp_logout_url();?>";
	var logoutURL = logoutURLOrg.replace('&amp;','&');
	$('.widget_links li').find('a[href $="logout"]').attr('href',logoutURL);
})(jQuery);
</script>
<?php
}
...
/*
 * wp_authenticate_userにfilterかけて、エクストラ情報を保存
 */
//mu-plugin『global_login_redirect』へ移動


/*
 * 一時的なメールテスト 0805（?mailtest=xxx）
 * wp_mail( $to, $subject, $message );
 */

//fromのアドレス＆名前変更
add_filter( 'wp_mail_from', function ( $hoge ) {return 'noreply@yaritori.net';} );
add_filter( 'wp_mail_from_name', function ( $hoge ) {return 'yaritori';} );

//送信
// add_action( 'init', 'pm_mailtest_postfix_ses', 100 );
// function pm_mailtest_postfix_ses(){

// 	$mails = array(
// 		'hiroshi_sekiguchi@pressman.ne.jp',
// 		'manabu_sekiguchi@pressman.ne.jp',
// 		'shigeru_sekiguchi@pressman.ne.jp',
// 		'taichi_nakamura@pressman.ne.jp',
// 		'nahoko_hagiwara@pressman.ne.jp',
// 		'takahisa_kageyama@pressman.ne.jp',
// 		'hideya_abe@pressman.ne.jp',
// 		'toshiro_shimizu@pressman.ne.jp',
// 		'yumiko_naoe@pressman.ne.jp',
// 		'yuna_hino@pressman.ne.jp',
// 		'gdo_research@pressman.ne.jp',
// 		'haruka88happy@gmail.com',
// 		'pressman.inc@gmail.com'
// 	);

// 	// if( !isset($_GET['address']) ){return;}

// 	//mailtestの数だけ一人にメールがいく
// 	if( isset($_GET['mailtest']) && intval($_GET['mailtest']) > 0 ){
// 		$howMany = intval($_GET['mailtest']);
// 	} else {
// 		return;
// 	}
...
// 			$alias = '+testmail'.$i;
// 			$to = str_replace("@", $alias."@", $mail);
// 			if( $to !== '' ){
// 				wp_mail( $to, 'SES TEST from Hiroshi (' .$i. ')', "テストです。\n\r".date_i18n('Y.m.d H:i:s'));
// 			}
