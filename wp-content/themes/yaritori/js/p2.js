...
/*PM HS*/
			var queryString = ajaxReadUrl +'&action=get_latest_comments&load_time=' + pageLoadTime + '&lcwidget=' + lcwidget+ '&uid=' + uid;
...
/*PM HS*/
//ユーザーIDも渡す（見れるもの判定のため）
//			var queryString = ajaxReadUrl +'&action=get_latest_posts&load_time=' + pageLoadTime + '&frontpage=' + isFirstFrontPage + '&vp=' + postsOnPageQS + '&uid=' + uid;
//ユーザーID + 今いるカテゴリ情報を特定
			var currentCatID = $('input#p2-new-post-category').val();
			var queryString = ajaxReadUrl +'&action=get_latest_posts&load_time=' + pageLoadTime + '&frontpage=' + isFirstFrontPage + '&vp=' + postsOnPageQS + '&uid=' + uid + '&currentcadid=' + currentCatID;
			ajaxCheckPosts = $.getJSON(queryString, function(newPosts){

				if (newPosts != null) {

//カテゴリにいる時にはnewPosts.htmlをpost毎に分離して、見ていいものだけを抽出してnewPosts.htmlに戻す
if( currentCatID > 1){
	var newFilteredPosts =[];
	var tmpPosts = newPosts.html.split("<!--sep-->");
// PM HS 0708
	var regex = new RegExp('^<li ([^<>]*) class="([^"<>]*)category-id-' + currentCatID + '([^"<>]*)">');
	if( tmpPosts.length > 0 ){
		for (var i = 0, len = tmpPosts.length; i < len; i++) {
// PM HS 0708
			var tmpHtml = tmpPosts[i];
			var found = tmpHtml.match(regex);
			if( found ){
				newFilteredPosts.push(tmpPosts[i]);
			}
		}
	}
	newPosts.html = newFilteredPosts.join("");
}

					pageLoadTime = newPosts.lastposttime;
//TOP以外では自動出現しない（それ以外では通知のみ）
//					if (!isFirstFrontPage || (typeof newPosts.html == "undefined") ) {
//カテゴリでも自動出現する（authorの場合にはそもそもfetchに行かないので除外）
//					if ( (!isFirstFrontPage && !isFirstCategory) || (typeof newPosts.html == "undefined") ) {
//カテゴリにいて該当postが無い場合も考慮
					if ( (!isFirstFrontPage && !isFirstCategory) || (typeof newPosts.html == "undefined" || newPosts.html == "") ) {
...
/*PM HS*/
			var post_category = $('#p2-new-post-category').val();
/*PM HS 0624*/
			var $groupMailChecked = $('[name=groupMail]:checked');
			var groupMail = $groupMailChecked.map(function(index, el) { return $(this).val(); });
			if( typeof groupMail[0] !== "undefined"){
				groupMailFlag = 1;
			} else {
				groupMailFlag = 0;
			}
...
/*PM HS*/
		...
		//ファイルアップロード taichi
		var files = [];
		$("#files div.file").each(function(e){
...
//			var args = {action: 'new_post', _ajax_post:nonce, posttext: posttext, tags: tags, post_format: post_format, post_title: post_title, post_citation: post_citation, post_subscribe: post_subscribe , post_category: post_category ,"files": files,};
/*PM HS 0624*/
			var args = {action: 'new_post', _ajax_post:nonce, posttext: posttext, tags: tags, post_format: post_format, post_title: post_title, post_citation: post_citation, post_subscribe: post_subscribe , post_category: post_category ,"files": files,groupMailFlag: groupMailFlag};
...
(function( $ ){
    //File Upload by Taichi
    jQuery(document).ready(function($) {

        //var thisFormElements = $('#comment, #comment-submit, :input',$("#new_post")).not('input[type="hidden"]');
        var thisFormElements = $('#submit').not('input[type="hidden"]');
        var progressNum = 0;
        var enableSubmitBtn = function() {
            progressNum += 1;
            thisFormElements.prop('disabled', true).addClass('disabled');
        };
        var disableSubmitBtn = function() {
            progressNum -= 1;
            if ( progressNum === 0 ) {
                thisFormElements.prop('disabled', false).removeClass('disabled');
            }
        };
        ...
        var progressBar = $('<div class="progress-box"><div class="progress-bar"/></div>');
        $('#fileupload').fileupload({
            url: url,
            formData: {"action": "new_post", "sub_action": "upload"},
            dataType: 'json',
            autoUpload: true,
            acceptFileTypes: /(\.|\/)(gif|jpe?g|png|pdf|xls|xlsx|ppt|pptx|doc|docx|txt)$/i,
            ...
            if (file.error) {
                var errorText = file.error;
                if (file.error === 'File type not allowed') {
                    errorText = "ご指定のファイルタイプはアップロード出来ません。";
                }
                alert(errorText);
                data.context.remove();
                disableSubmitBtn();
            ...
            data.context.find('.progress-box').remove();
            data.context.data('result', result);
