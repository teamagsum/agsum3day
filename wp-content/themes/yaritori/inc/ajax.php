...
class P2Ajax extends P2Ajax_Read {
	 ...
	 */
	function new_post() {
		//fileupload 20140613 Taichi
        //$tmpDir = '/tmp/p2/';
		if ($_POST['sub_action'] == 'upload') {
            ...
            if(($post_max_size && ($content_length > $post_max_size)) || ($upload_max_size && ($content_length > $upload_max_size))) {
                print json_encode(["error" => "アップロードファイルのサイズ制限を超えています。" . $content_length . "|" . $post_max_size . "|" . $upload_max_size]);
                exit();
            }
			...
			if ( empty($result['file']) ) {
				die( '<p>ファイルのアップロードに失敗しました。</p>'. $result['error'] );
			}
			...
			$result["attach_data"] = $attach_data;
			$result["attach_id"] = $attach_id;



            //$tmpFile = sha1($file['tmp_name'] . microtime(). microtime());
			//move_uploaded_file($file['tmp_name'],$tmpDir . $tmpFile);
		...
		//ファイルがアップロード済みだったらファイルを保存して表示させる。 20140613 Taichi
		if( $_POST['files'] && is_array($_POST['files'])) {
			foreach ($_POST['files'] as $f) {
				$thumb = $f['attach_data']['sizes']['medium'];
				if( !isset($f['attach_data']['sizes']['medium']) ){
					$thumb = $f['attach_data']['sizes']['thumbnail'];
				}
				$url = dirname($f['url']) . '/' . $thumb['file'];
                $file_content = "";
                if(substr($f['type'],0,6) == "image/") {
				    $file_content = '<br /><img src="' . $url . '" alt="' . $thumb['file'] . '" width="' . $thumb['width'] . '" height="' . $thumb['height'].'" />';
                } else {
                    $file_content = basename($f['url']);
                }
                $file_content = '<br /><a href="' . $f['url'] . '">' . $file_content . '</a>';
		...
		//PM HS 0619
		if($post_category < 1){
			$post_category = intval($_POST['cat']);
		}
