...
class P2Ajax_Read {
	...
	function get_latest_posts() {
		...
		/*PM HS 0618 今いるカテゴリ(単数)*/
		$currentCatID = (int)($_GET['currentcadid']);

		//このユーザーが見れるカテゴリ(csv)
		//pm_get_mygroupIDsが無くても動作するように 20150704 PM HS
		if( function_exists('pm_get_mygroupIDs') ) {
			$avairable_cat_IDs = pm_get_mygroupIDs($user_ID);
		} else {
			$avairable_cat_IDs = 1;
		}

		//今いるカテゴリが見れるカテゴリに含まれなかったらNG
		$tmpArray = explode(',' , $avairable_cat_IDs);
		if( !in_array( $currentCatID , $tmpArray) ){
			exit;
		}

		//topにいるなら見れるもの全て、カテゴリにいるなら今いるカテゴリだけの投稿取得
		if( $currentCatID > 1 ){
			// $groupfilter  = '&cat='.$currentCatID;
			$groupfilter  = '&cat='.$avairable_cat_IDs;//カテゴリにいても見れるもの全て全部とる
		} elseif ( $currentCatID == 1 ){
			$groupfilter  = '&cat='.$avairable_cat_IDs;
		} else {
			//pm_get_mygroupIDsが無くても動作するように 20150704 PM HS
			$groupfilter  = '';
			//exit;
		...
		//グループフィルター追加
		query_posts( 'showposts=' . $num_posts . '&post_status=publish' . $groupfilter );
		ob_start();
